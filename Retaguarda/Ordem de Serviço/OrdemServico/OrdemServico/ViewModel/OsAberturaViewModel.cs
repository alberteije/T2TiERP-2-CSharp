...
{
    public class OsAberturaViewModel : ViewModelBase
    {

        #region Variáveis

        public ObservableCollection<OsAberturaDTO> ListaOsAbertura { get; set; }
        private OsAberturaDTO _OsAberturaSelected;
        private bool _isEditar { get; set; }

        #endregion

        #region Construtor
        public OsAberturaViewModel()
        {
            try
            {
                ListaOsAbertura = new ObservableCollection<OsAberturaDTO>();
                IndiceNavegacao = 0;
        ...
        #region Infra
        public OsAberturaDTO OsAberturaSelected
        {
            get { return _OsAberturaSelected; }
            set
            {
                _OsAberturaSelected = value;
                notifyPropertyChanged("OsAberturaSelected");
            }
        }
        ...
        #region CRUD
        public void SalvarAtualizarOsAbertura()
        {
            try
            {
                using (ServidorClient Servico = new ServidorClient())
                {
                    Servico.SalvarAtualizarOsAbertura(OsAberturaSelected);
        ...
        public void AtualizarListaOsAbertura(int pagina)
        {
            try
            {
                using (ServidorClient Servico = new ServidorClient())
                {
                    if (pagina == 0)
                        IndiceNavegacao = 0;
                    else if (pagina > 0 && ListaOsAbertura.Count == QuantidadePagina)
                        IndiceNavegacao += QuantidadePagina;
                    else if (pagina < 0 && IndiceNavegacao != 0)
                        IndiceNavegacao -= QuantidadePagina;

                    OsAberturaDTO OsAbertura = new OsAberturaDTO();
                    if (!Filtro.Trim().Equals(""))
                    {
                        OsAbertura.Numero = Filtro;
                    }

                    IList<OsAberturaDTO> ListaServ = Servico.SelectOsAberturaPagina(IndiceNavegacao, QuantidadePagina, OsAbertura);

                    ListaOsAbertura.Clear();

                    foreach (OsAberturaDTO objAdd in ListaServ)
                    {
                        ListaOsAbertura.Add(objAdd);
                    }
                    OsAberturaSelected = null;
                }
                QuantidadeCarregada = ListaOsAbertura.Count;
                ControlarNavegacao();
            }
            catch (Exception ex)
            {
        ...
        public void BotaoInserir()
        {
            try
            {
                OsAberturaSelected = new OsAberturaDTO();
                IsEditar = true;
            }
            catch (Exception ex)
            {
            ...
            try
            {
                if (OsAberturaSelected != null)
                {
                    if (MessageBox.Show("Tem Certeza Que Deseja Excluir o Registro?", "Pergunta do Sistema", MessageBoxButton.YesNo, MessageBoxImage.Question) == MessageBoxResult.Yes)
                    {
                        ...
                        AtualizarListaOsAbertura(IndiceNavegacao);
                    }
                }
                else
                    MessageBox.Show("Selecione um elemento na lista.", "Alerta do sistema", MessageBoxButton.OK, MessageBoxImage.Information);
            ...
            get
            {
                if (BotaoExportarXlsCommand == null)
                {
                    ...
                    (
                        param =>
                        {
                            DataGridExportacao.ItemsSource = ListaOsAbertura;
